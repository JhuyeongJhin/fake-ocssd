...
static int user_pin = 1;						// 0 unpin,  1 pin
module_param(user_pin, int, S_IRUGO);
MODULE_PARM_DESC(user_pin, "user pinning");

static int wt_pin = 1;							// 0 unpin,  1 pin
module_param(wt_pin, int, S_IRUGO);
MODULE_PARM_DESC(wt_pin, "pblk-writer pinning");

static int rb_choice = 1;						// 1 rand,  2 max (remain)
module_param(rb_choice, int, S_IRUGO);
MODULE_PARM_DESC(rb_choice, "ring buffer choice");

static int rb_gc_choice = 1;						// 1 rand,  2 max (remain)
module_param(rb_gc_choice, int, S_IRUGO);
MODULE_PARM_DESC(rb_gc_choice, "ring buffer choice of GC");
...
*/
complete:	
//	printk("JJY: queue_rq %d", rqd->ppa_addr);
...
/*	
	id->nr_rwb = nr_rwb;
	id->user_pin = user_pin;
	id->wt_pin = wt_pin;
	id->wt_nice = wt_nice;
	id->rb_choice = rb_choice;
	id->rb_gc_choice = rb_gc_choice;
*/	
	id->ver_id = 0x1;
	id->vmnt = 0;
	id->cgrps = 1;
	...
	num_lun = grp->num_lun = 1;		/*luns per chnl, dev->luns_per_chnl*/
...
//	printk("JJY: submit_io %d %d", smp_processor_id(), rqd->ppa_addr);
